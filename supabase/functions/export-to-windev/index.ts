...
serve(async (req) => {
  ...
  try {
    ...
    // Get WinDev configuration
    const { data: config, error: configError } = await supabaseClient
      .from('platform_configurations')
      .select('*')
      .eq('user_id', user.id)
      .eq('platform_type', 'windev')
      .eq('is_active', true)
      .single();

    if (configError || !config) {
      throw new Error('WinDev configuration not found');
    }
    ...
    for (const analysis of analyses) {
      try {
        const productData = mapAnalysisToWinDev(analysis, fieldMappings || [], pricingRules || [], config);

        // Post to WinDev API
        const response = await fetch(`${config.platform_url}/api/products`, {
          method: 'POST',
          headers: {
        ...
        if (!response.ok) {
          throw new Error(`WinDev API error: ${response.statusText}`);
        }

        const result = await response.json();
        results.push({ analysis_id: analysis.id, success: true, windev_id: result.id });

        // Log successful export
        await supabaseClient.from('platform_export_logs').insert({
          user_id: user.id,
          platform_type: 'windev',
          products_count: 1,
          success_count: 1,
          error_count: 0,
          export_details: { analysis_id: analysis.id, windev_id: result.id }
        });
      } catch (error) {
...
function mapAnalysisToWinDev(
  ...
  const productData: any = {
    Nom: analysisData.basic_info?.product_name || 'Produit sans nom',
    Description: analysisData.seo_content?.product_description || '',
    Prix: finalPrice.toFixed(2),
    Categorie: analysis.mapped_category_name || 'Général',
    Actif: true
  };
