...
serve(async (req) => {
  ...
  try {
    ...
    // Get Just Eat configuration
    const { data: config, error: configError } = await supabaseClient
      .from('platform_configurations')
      .select('*')
      .eq('user_id', user.id)
      .eq('platform_type', 'just_eat')
      .eq('is_active', true)
      .single();

    if (configError || !config) {
      throw new Error('Just Eat configuration not found');
    }
    ...
    for (const analysis of analyses) {
      try {
        const productData = mapAnalysisToJustEat(analysis, fieldMappings || [], pricingRules || [], config);

        // Post to Just Eat API
        const response = await fetch(`${config.platform_url}/api/v2/products`, {
          method: 'POST',
          headers: {
        ...
        if (!response.ok) {
          throw new Error(`Just Eat API error: ${response.statusText}`);
        }

        const result = await response.json();
        results.push({ analysis_id: analysis.id, success: true, just_eat_id: result.id });

        // Log successful export
        await supabaseClient.from('platform_export_logs').insert({
          user_id: user.id,
          platform_type: 'just_eat',
          products_count: 1,
          success_count: 1,
          error_count: 0,
          export_details: { analysis_id: analysis.id, just_eat_id: result.id }
        });
      } catch (error) {
...
function mapAnalysisToJustEat(
  analysis: ProductAnalysis,
  fieldMappings: any[],
  pricingRules: any[],
  config: JustEatConfig
): any {
