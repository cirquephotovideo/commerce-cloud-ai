-- Fix merge_duplicate_analyses_by_ean with better duplicate handling using EXCEPTION
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      BEGIN
        -- Try to insert with the new analysis_id
      ...
      EXCEPTION WHEN unique_violation THEN
        -- If duplicate exists, just delete this one
        DELETE FROM product_links WHERE id = link_record.id;
      END;
    END LOOP;
    
    -- Clean up any remaining orphaned links from duplicates
